...
class Receiver {
  ...
  static Future<List<SenderModel>> scan() async {
    ...
    // //循环整个子网网段来查找是否有设备上线  三哥这代码写得简直离谱
    // for (int i = 2; i < 255; i++) {
  ...
  static Future<Map<String, dynamic>> _connect(String host, int port) async {
    try {
      var socket = await Socket.connect(host, port).timeout(const Duration(milliseconds: 2500));
      socket.destroy();
  ...
  static receive(SenderModel senderModel, int secretCode) async {
    PercentageController getInstance = GetIt.instance.get<PercentageController>();
    //getting hiveObj
  ...
  static getFile(String filePath,int fileIndex,SenderModel senderModel) async {
    Dio dio = Dio();
    ...
    try {
      ...
      sendBackReceiverRealtimeData(senderModel,fileIndex: fileIndex, isCompleted: false);
      stopwatch.start();
